...
**Paper:** Haeri Boroujeni et al. (Clemson University), "Don't Waste Bits! Adaptive KV-Cache Quantization for Lightweight On-Device LLMs", arXiv 2604.04722, 2026.

**What it claims:** Importance-driven per-token/per-block bit-width allocation for the KV cache: high-importance tokens retain higher precision, low-importance ones drop to 2-3 bits with small quality loss. Important caveats: the paper's allocation policy is **learned** (a trained predictor), and its evaluation is on SmolLM-class on-device models. It does not evaluate composition with rotation-codebook quantizers like TurboQuant — any "compounds with TurboQuant" claim is ours to prove, not theirs.

**Lambda status:** ✓ NEW — TIU block added in Phase 0.3. The TIU is **inspired by** this paper's adaptive-precision idea, not an implementation of it: Lambda replaces the learned policy with a cheap attention-weight accumulator (cumulative softmax mass per 16-token block) feeding threshold-based precision/eviction decisions.
...
**Paper:** Zandieh, Daliri, Hadian, Mirrokni (Google), "TurboQuant: Online Vector Quantization with Near-optimal Distortion Rate", arXiv 2504.19874, ICLR'26.

**What it claims:** Random orthogonal rotation over the **full vector dimension** — generated by QR decomposition of an i.i.d.-Gaussian matrix, **not a Hadamard transform** ("We can generate Π by applying QR decomposition on a random matrix with i.i.d Normal entries") — concentrating coordinates toward a near-Gaussian/Beta distribution, + per-coordinate optimal scalar (Lloyd-Max) quantizers with **no per-group scale factors**, + **two-tier channel-wise bit allocation** (paper's explicit example is the 2.5-bit setting: 32 outlier channels @ 3 bits, 96 @ 2 bits over d=128; the 3.5-bit split is "a different ratio", unstated), + a 1-bit Quantized-JL transform on the residual for unbiased inner products. Quality-neutral KV compression at 3.5 bits/channel (LongBench-E + needle-in-haystack on Llama-3.1-8B-Instruct and Ministral-7B-Instruct; no MMLU reported). The paper reports **no experiments below d=200**, and its concentration/near-independence arguments explicitly require high dimension. *(Mechanism re-verified against the arXiv body 2026-06-10 — an earlier pass here said "randomized Hadamard"; the paper's stated construction is QR-based.)*

**Lambda status:** ✓ KCE-mini implements a 16-element block restructuring of the rotation-plus-codebook recipe — a **different quantizer** from the published one: uniform 3-bit codebook + per-block scale, 16 × 3 + 16 = 64 bits per 16 elements = 4.0 bpe → 4.0× compression vs FP16 (rate arithmetic verified by re-derivation 2026-05-14). Quality at 4.0 bpe is a design target pending the pre-HLS golden-model eval, not a published result.
...
**Open question:** Asymmetric K3V2 mode (K @ 4.0 bpe, V @ 3.0 bpe avg 3.5 bpe → 4.57× compression). Provenance correction: this is **Lambda's own extension**, motivated by published K-vs-V sensitivity findings (K is more quantization-sensitive than V — KVQuant et al., arXiv 2401.18079); it is *not* a TurboQuant mode, and the earlier "TurboQuant §5" attribution was wrong. Confirmed as a CSR-selectable mode in `arch.yml`; quality unvalidated alongside the primary mode.

**Notes for visual rep:** "KCE-mini: 16-pt Hadamard → Lloyd-Max 8-centroid → bit-pack. Five CSR modes: TurboQuant-style 3-bit (primary), Hadamard-INT4 (linear fallback), Asymmetric K3V2 (Lambda extension), FP4 codebook (NVFP4 levels), FP16 bypass (debug)."
...
**Paper:** Li, Haoyang, et al., "A Survey on Large Language Model Acceleration based on KV Cache Management", arXiv 2412.19442, 2024 (TMLR 2025). (An earlier draft of this entry cited arXiv 2506.13131, which is a different, unrelated paper; corrected 2026-06-10.)

**Status:** not yet read. Highest-value survey for cross-checking compression choices vs the broader landscape; to be read in full during Phase B execution.
