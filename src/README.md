...
> **Codec-of-record note (2026-06-22 pivot):** the KV codec of record is **ChannelQuant** (per-channel INT4 K + per-token INT4 V + static top-k FP16 outlier lane; tiers CQ-8/CQ-4/CQ-4+), packaged as the **KV Cache Engine (KVE)** — full block in the `kv-cache-engine` repo. The `blocks/kce/` directory and any `kce` code identifiers are kept as-is for HLS continuity, but the block is the **KVE**; the TurboQuant/Hadamard microarchitecture and its PPA numbers referenced in these READMEs predate the pivot and are pending re-derivation.
...
2. **KVE ChannelQuant datapath** — the headline research IP (KV Cache Engine; per-channel INT4 K + per-token INT4 V + static top-k FP16 outlier lane; tiers CQ-8/CQ-4/CQ-4+). Validate bit-exactly against the Python golden in `golden/kce.py`. *(The prior "KCE-mini Hadamard butterfly + Lloyd-Max classifier + bit-pack, 0.08 mm², 5 TurboQuant CSR modes" description is legacy pre-pivot and pending re-derivation; the `kce.*` file names are kept for HLS continuity.)*
