...
- **16 KB microcode RAM** — up to 4K 32-bit instructions; holds the entire pre-compiled model schedule for one transformer
...
Compiler emits the schedule once per model; chip walks it forever. The host loads ~3K instructions of microcode (of the 4K capacity) over PCIe at boot. Layer N+1 reuses layer N's schedule with only the layer-index register bumped. No need for branch prediction or speculative execution at this scale.
