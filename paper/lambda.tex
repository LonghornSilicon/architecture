...
\textbf{(2) Per-channel dequantization before scoring.} The datapath serializes one shared FP16 compute unit across the $D$ head-dimension channels (a single divide cone), and keys are reconstructed per-channel (INT4\,$\cdot$\,FP16, with FP16 replay for the $k{=}2$ outlier channels) \emph{before} the attention dot product. Lambda's matrix engine (MatE) therefore needs no FP16 multiplier datapath for weights (INT8\,$\times$\,INT4) and scores Q\,$\cdot$\,K$^\top$ on the dequantized K; there is no compressed-domain / raw-index read path. FP16 is confined to VecU (softmax, RMSNorm, RoPE, SiLU) and the KVE's shared scale unit.
...
ChannelQuant makes this gating unnecessary by construction. Because keys are quantized \emph{per channel}, each channel's dynamic range is captured by its own FP16 scale, so a large-magnitude channel does not force a coarse quantizer on the rest of the tile. The $k{=}2$ channels that would still degrade INT4---the static outliers selected by the calibrated ROM mask---are simply held in FP16 in the outlier lane. There is thus no run-time ``outlier tile'' to route: the outlier handling is static, and the KVE dequantizes keys per-channel to FP16 \emph{before} the array. The empirical evidence is in the codec's accuracy: HellaSwag acc\_norm within $\approx$0.4--0.8\,pt of FP16 at the CQ-4+ tier on Qwen2-0.5B/1.5B, consistent with KIVI~\cite{kivi} and KVQuant~\cite{kvquant}. We rely on this to omit the FP16 multiplier from MatE entirely; an FP16 path remains in VecU (softmax, RMSNorm, RoPE, transcendentals) and in the KVE's shared scale unit.
