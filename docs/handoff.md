...
> **Codec-of-record note:** the KV codec of record is **ChannelQuant** (per-channel INT4 K grouped G=128 + per-token INT4 V + static top-k k=2 FP16 outlier lane; tiers CQ-8/CQ-4/CQ-4+; ~3.8× at ~4 bits/value), packaged as the **KV Cache Engine (KVE)** — full block RTL complete through Sky130 sign-off in the `kv-cache-engine` repo. Keys are dequantized per-channel (`INT4·FP16`) before the score matmul; there is no compressed-domain read path. The KVE's 16nm PD numbers (area/power/Fmax) are TBD — pending re-measurement for ChannelQuant; per-model throughput/capacity/accumulator numbers derived for the retired 3-5B target are TBD — pending re-derivation for ChannelQuant / Qwen2-1.5B. TurboQuant is cited prior work only.

**Lambda** is a **4 mm² standalone transformer-decode ASIC** on **TSMC 16nm FinFET (N16FFC)**, designed for tape-out via the **IMEC / Europractice mini@sic 2.0** academic shuttle (~\$60–100K shuttle, ~\$170–290K total chip cost). It runs **up to 1.5B-parameter W4A8 LLMs (validated on Qwen2-1.5B)** at **6–8 tok/s decode in 2.6 W typical**, with the KVE ChannelQuant KV codec (keys dequantized per-channel before scoring), and a Token Importance Unit (TIU) driving adaptive-precision KV.
...
Lambda integrates **seven on-die functional blocks** under the canonical four-block taxonomy (ACU / KVE / TIU / MHC) plus LSU control and HIF host I/O. The **ACU** (Block 1) groups MatE (8×8 INT8×INT4 + FP16 (P·V) systolic with INT24 K-axis accumulator — INT8×INT4 for weight/FFN GEMMs, with a per-tile FP16 P·V mode selected by the ACU precision controller) and VecU (8-lane FP16/BF16 SIMD running FA-3 online-softmax + RoPE + RMSNorm + SiLU microcode). The **KVE** (Block 2, KV Cache Engine) is the **ChannelQuant** KV codec — per-channel INT4 K (grouped, G=128, D FP16 scales) + per-token INT4 V + static top-k k=2 FP16 outlier lane; unified per-channel SRAM record; one serialized fp16 divide cone across D channels; decompress = per-channel `INT4·FP16`; ~3.8× at ~4 bits/value; tiers CQ-8/CQ-4/CQ-4+. Block RTL is complete through Sky130 sign-off in the `kv-cache-engine` repo; 16nm PD numbers are TBD. **MSC** (Block 4, maps to the canonical Memory Hierarchy Controller / MHC) provides PagedAttention via a 128-entry block table, a 4-port SRAM crossbar, and the LPDDR5X x16 protocol-side controller. LSU is a tiny 32-instruction in-order RISC walking the pre-compiled per-layer schedule from 4 KB of microcode RAM. **TIU (Block 3, new this session)** is a 0.03 mm² block that accumulates per-block attention entropy and drives both heavy-hitter eviction (to MSC) and per-block adaptive precision (to the KVE). HIF is a PCIe Gen3 x1 endpoint on the M.2 2280 carrier-card form factor. On-die SRAM is 0.8 MB across 4 banks (kv_scratchpad 0.4 + activation_buf 0.3 + weight_stream 0.05 + codebook ROM 64 KB). Off-die is a 4–8 GB LPDDR5X-8533 x16 package and a host laptop / dev board over PCIe.

**Targets at decode (LPDDR5X x16 baseline):** canonical target is up to 1.5B parameters, validated on **Qwen2-1.5B**. *(FLAG: the per-model throughput figures previously listed here — Llama-3.2-3B @ 7.4 tok/s, Mistral-NeMo-3B @ 8.0, Qwen2.5-3B @ 7.7, Phi-3.5-mini @ 6.3, Llama-3.2-1B @ 19.3 — are pre-pivot 3-5B derivations and pending re-derivation for the ≤1.5B target + ChannelQuant.)* **Power:** 2.6 W typical / 3.3 W peak. **Area:** 4.354 mm² gross / **4.014 mm² with recommended shrink path** (contingent on PHY vendor quote landing at 1.0 mm² best case).
...
**(2) INT24 K-axis accumulator numerical eval.** The accumulator width was derived from the retired 3B dims (FFN K up to 8192, where INT24 *just* fits: log₂(8192) + 11 = 24 bits). Under the canonical Qwen2-1.5B target the actual FFN K is smaller, but the **margin is TBD — pending re-derivation for Qwen2-1.5B.** Run a Python sweep on the target model's actual per-layer dimensions and confirm no saturation; may want INT28/INT32 for safety margin. Spec source: [`../arch.yml`](../arch.yml) MatE block, `accumulator_rationale` field.

**(3) Per-block optimization tests — well-defined success criteria.** Currently `src/blocks/<block>/README.md` has the spec but no measurable acceptance criteria. Each of the seven blocks (MatE, VecU, KVE, MSC, LSU, HIF, TIU) needs: target metric (cycles per op / GOPS / mm² / mW), test-vector source, pass/fail threshold, golden-model reference. Land this in each block README before HLS begins. Template suggestion: a "verification gates" subsection per block.
...
These are spec gaps that need to close before the HLS work (Phase E) can start cleanly. Each maps to a specific block in [`../src/blocks/`](../src/blocks/).
   ...
   **(6) ChannelQuant per-channel SRAM record layout in kv_scratchpad** — the codec of record uses a **unified per-channel record** `{tag, D×FP16 scale field, D×INT4 code}` for keys plus a per-token INT4 lane for values, and a separate FP16 lane for the k=2 outlier channels. The exact byte layout of this record inside the 0.4 MB kv_scratchpad (alignment, tag width, outlier-lane placement) is spec'd in the `kv-cache-engine` repo but needs mirroring into [`../arch.yml`](../arch.yml) / [`../src/blocks/msc/README.md`](../src/blocks/msc/README.md) for the on-die integration. Reference: KIVI (ICML 2024), KVQuant (2024), and the `kv-cache-engine` block spec.

   **(7) Outlier-channel ROM mask calibration** — the static top-k (k=2) outlier channels are selected by a **calibrated ROM mask**. The calibration procedure (which channels, per-model vs per-layer, how the mask is generated and loaded) is defined in the `kv-cache-engine` repo; document the on-die mask ROM format and load path in the const-ROM spec. Reference: KVQuant dense-and-sparse outlier handling.
   ...
   **(9) ChannelQuant quality eval on the target model** — the codec's accuracy is characterized in the `kv-cache-engine` repo (HellaSwag acc_norm within ~0.4–0.8 pt of FP16 at CQ-4+ on Qwen2-0.5B/1.5B). The remaining task is a **MMLU / LongBench / Needle-in-a-Haystack eval against Qwen2-1.5B at W4A8 + ChannelQuant (CQ-4 / CQ-4+)** to confirm the tier selection for the demo. Keys are dequantized per-channel (`INT4·FP16`) before scoring — there is no compressed-domain / raw-index read path to validate. Owner: ML student. Deadline target: 2026-07. Gates the demo-model decision.

   **(10) MLA-gap revisit decision** — currently locked out per [`../docs/literature_audit.md`](literature_audit.md) §3. **If DeepSeek-V2-style models become the dominant small-model target by the Summer 2027 tape-out, this gap is the chip's biggest competitive risk.** Worth a periodic check (every 3–6 months). The architectural cost to add MLA is real: MSC + MatE redesign for latent KV layout, ~+0.05 mm² across blocks, ~30 verification tests.
...
|---|---|---|
| 4 mm² die at TSMC 16nm FinFET (N16FFC), via imec / TSMC University Program | `arch.yml` `metadata` + `process` | IMEC mini@sic 2.0 minimum-block-area tier; ~\$60–100K academic shuttle. Anything bigger is unfundable. |
...
| ChannelQuant KV codec (KVE block) — per-channel INT4 K (G=128) + per-token INT4 V + top-k k=2 FP16 outlier lane; tiers CQ-8/CQ-4/CQ-4+ | `arch.yml` KVE block; `kv-cache-engine` repo | Codec of record. Block RTL complete through Sky130 sign-off. Recipe follows KIVI (ICML 2024) / KVQuant (2024). 16nm PD numbers TBD — pending re-measurement. |
...
| **Per-channel dequant before Q·K^T scoring** | `arch.yml` `attention_compute` | Under the ChannelQuant codec of record the KVE reconstructs K per-channel (`INT4·FP16`, + FP16 replay for the k=2 outlier channels) before the score matmul. There is no compressed-domain / raw-index read path. |
...
| **ACU naming convention** (umbrella = MatE+VecU+KVE) | `arch.yml` `compute_unit_grouping` | Honors Chaithu's framework; preserves internal block structure for HLS. |
...
| Demo target model — up to 1.5B, validated on Qwen2-1.5B | ML student quality eval (MMLU + LongBench) at W4A8 + ChannelQuant (CQ-4/CQ-4+) | 2026-07 |
...
| What's the bits/value / compression rate? | `arch.yml` KVE block (ChannelQuant: ~4 bits/value, ~3.8× vs FP16) |
...
3. **W4 quantization quality on the ≤1.5B / Qwen2-1.5B target** — literature says a few % MMLU degradation acceptable for demo; needs eval on the specific demo target model.
4. **INT24 sufficiency at the target model's FFN K** — accumulator width was derived from the retired 3B dims; margin for Qwen2-1.5B is TBD — pending re-derivation. Sanity-check before HLS (TODO §3.2 item 2).
5. **ChannelQuant tier selection on the target model** — accuracy is characterized (near-lossless at CQ-4+); the remaining eval is CQ-4 vs CQ-4+ on Qwen2-1.5B for the demo (TODO §3.3 item 9). 16nm PD numbers are TBD — pending re-measurement.
...
7. **Tape-out timeline (Summer 2027)** — model class may commoditize on mobile NPUs by then; mitigation is the open-source-academic-standalone differentiation, which doesn't compete on raw perf.
