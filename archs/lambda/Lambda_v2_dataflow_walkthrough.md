# Lambda v2 — Unit-by-Unit Dataflow Walkthrough

A guided tour of every block in Lambda v2, walked through in the order data actually flows during a decode token. Each block becomes concrete as it appears in the journey of one token through one layer.
...
**Companions:**
- [`Lambda_v2_4mm2.yaml`](Lambda_v2_4mm2.yaml) — machine-readable spec with all numbers
- [`Lambda_v1_4mm2.yaml`](Lambda_v1_4mm2.yaml) — the conservative coprocessor alternative
- [`../../STATUS.md`](../../STATUS.md) — v1-vs-v2 comparison and decision rationale
- [`../../scripts/v2_design_space/`](../../scripts/v2_design_space/) — Python validation of every number used here

---

## Stage 0 — Before any decode: the boot

When you plug Lambda into your laptop's USB-C port, the very first block to wake up is the **HIF (Host Interface)**. It's a USB-C 2.0 endpoint — a single physical port on the chip that runs at 480 Mbps. Inside the chip, HIF sits at the perimeter of the die (a ~0.30 mm² block) and has three responsibilities at boot: enumerate as a USB device on your host, expose a CSR (Configuration / Status Register) interface so the host driver can poke the chip's control registers, and provide a JTAG + scan-chain debug port for if something breaks.

The host driver does two things over HIF: (1) writes ~3K instructions of microcode into the LSU's instruction RAM (this is the pre-compiled schedule for "run Llama-3.2-3B"), and (2) initiates a 1.5 GB DMA transfer of the W4-quantized weights from your laptop's RAM into the chip's external LPDDR5X package. That second step takes ~25 seconds at 60 MB/s, but it's a one-time cost per power-on. After that, weights live off-chip in DRAM forever.
...
The **MatE (Matrix Engine)** is 0.10 mm² of pure compute: an 8×8 grid of identical PEs (Processing Elements), each containing one INT8×INT4 multiplier, one INT16 accumulator, and a few latches.
...
The Lloyd-Max codebook is 8 centroids in a 64-byte ROM, optimal for the Beta distribution. Each coordinate independently picks the nearest centroid — that's a 3-bit index. We pack 16 of those plus a 16-bit magnitude header into 64 bits, hitting ~3 bits/element effective. Compression vs FP16: 3.0× (the trade-off for using 16-point instead of flagship's 32-point butterfly to save area).
...
HIF picks it up, packetizes it as USB-C bulk transfer, ships it over the cable to your laptop. Your laptop's driver hands it to whatever runtime is reading from the chip (a llama.cpp backend, a custom Python loop, whatever). Your screen prints "the".
